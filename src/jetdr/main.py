...
app = typer.Typer(
    name="jetdr",
    help="DaVinci Resolve 自動編集エージェント - 無音区間・フィラー自動検知・削除ツール",
...
def version_callback(value: bool) -> None:
    """バージョン表示"""
    if value:
        console.print(f"jetDR version {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """jetDR - DaVinci Resolve 自動編集エージェント"""
    pass
...
def process(
    ...
    """
    from jetdr.audio.analyzer import SilenceDetectionError
    from jetdr.audio.extractor import AudioExtractionError
    from jetdr.cli.config_loader import load_app_config
    from jetdr.cli.processor import process_audio
    from jetdr.cli.summary import display_summary
    from jetdr.davinci.connection import DaVinciConnectionError
    from jetdr.exceptions import ExportError
    from jetdr.exporters import (
        ExportConfig,
        LiveConnectionExporter,
        create_exporter,
    )
    from jetdr.speech.transcriber import TranscriptionError
    from jetdr.utils.file_utils import validate_input_file
    from jetdr.utils.logger import get_logger, setup_logger
        ...
        with Progress(
            ...
            if not dry_run:
                ...
                try:
                    ...
                    if isinstance(exporter, LiveConnectionExporter):
                        with exporter:
                            if exporter.is_connected:
                                ...
                                console.print(
                                    f"[red]Error:[/red] Could not connect to {target}"
                                )
                                console.print(
                    ...
                    console.print(f"[red]Error:[/red] Unknown exporter target: {target}")
...
def analyze(
    ...
    from jetdr.cli.config_loader import load_app_config
    from jetdr.cli.summary import display_analysis_table
...
def batch(
    ...
    フォルダ内の全動画を一括処理する
    """
    from jetdr.utils.file_utils import list_video_files
