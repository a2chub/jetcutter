...
環境変数による設定上書きをサポート:
    JETDR_DEVICE: デバイス設定 (auto, cuda, cpu)
    JETDR_COMPUTE_TYPE: 計算精度 (auto, float16, int8)
    JETDR_MODEL_NAME: Whisperモデル名
    JETDR_LANGUAGE: 認識言語コード
    JETDR_THRESHOLD_DB: 無音判定しきい値
    JETDR_FPS: フレームレート
    JETDR_LOG_LEVEL: ログレベル (DEBUG, INFO, WARNING, ERROR)
"""
...
class SilenceDetectionConfig(BaseModel):
    ...
    threshold_db: float = Field(
        default_factory=lambda: _get_env_float("JETDR_THRESHOLD_DB", -40.0),
...
class FillerDetectionConfig(BaseModel):
    ...
    model_name: str = Field(
        default_factory=lambda: _get_env_str("JETDR_MODEL_NAME", "large-v3"),
        description="Whisperモデル名（tiny, base, small, medium, large-v3等）",
    )
    language: str = Field(
        default_factory=lambda: _get_env_str("JETDR_LANGUAGE", "ja"),
        description="認識対象言語コード",
    )
    device: str = Field(
        default_factory=lambda: _get_env_str("JETDR_DEVICE", "auto"),
        description="使用デバイス（auto, cuda, cpu）",
    )
    compute_type: str = Field(
        default_factory=lambda: _get_env_str("JETDR_COMPUTE_TYPE", "int8"),
...
class AppConfig(BaseModel):
    ...
    fps: float = Field(
        default_factory=lambda: _get_env_float("JETDR_FPS", 29.97),
