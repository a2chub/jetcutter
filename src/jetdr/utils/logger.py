...
環境変数:
    ...
    JETDR_LOG_FORMAT: ログフォーマット（text, json）
    JETDR_LOG_DIR: ログファイル出力ディレクトリ
"""
...
def _get_log_level_from_env() -> str:
    """環境変数からログレベルを取得"""
    level = os.environ.get("JETDR_LOG_LEVEL", "INFO").upper()
...
def _get_log_format_from_env() -> str:
    """環境変数からログフォーマットを取得"""
    fmt = os.environ.get("JETDR_LOG_FORMAT", "text").lower()
...
def _get_log_dir_from_env() -> Path | None:
    """環境変数からログディレクトリを取得"""
    log_dir = os.environ.get("JETDR_LOG_DIR")
    if log_dir:
        return Path(log_dir)
    return None
...
def setup_logger(
    ...
    if log_dir is not None:
        ...
        if json_format:
            logger.add(
                log_path / "jetdr_{time:YYYY-MM-DD}.jsonl",
                format="{message}",
                level="DEBUG",
                rotation=rotation,
                retention=retention,
            ...
            logger.add(
                log_path / "jetdr_{time:YYYY-MM-DD}.log",
